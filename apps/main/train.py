...
from lingua.args import dataclass_from_dict, dump_config, flatten_dict, load_config_file
...
def validate_train_args(args: TrainArgs, output_size: int):
    ...
    if args.logging.wandb is not None:
        ...
        if len(wandb_name) > 128:
            # Create a deterministic seed from the full name
            name_seed = sum(ord(c) for c in wandb_name)
            name_rng = random.Random(name_seed)
            rand_infill_str = "".join(name_rng.choices(string.ascii_letters + string.digits, k=3))
            wandb_name = wandb_name[:58] + f"_[omit_{rand_infill_str}]_" + wandb_name[-58:]
        
        args.logging.wandb.name = wandb_name
        args.logging.wandb.id = wandb_name

        tags = []
        for tag in args.logging.wandb.tags:
            if len(tag) > 64:
                tag = tag[:64]
            tags.append(tag)
        
        args.logging.wandb.tags = tags
...
def train(args: TrainArgs):
    with ExitStack() as context_stack:
        ...
        if args.checkpoint.init_ckpt_path:
            ...
            if args.checkpoint.load_optim_from_init_ckpt:
                logger.info(f"Loading both the model and the optimizer from the checkpoint.")
                load_from_checkpoint(args.checkpoint.init_ckpt_path, model, optimizer, model_key="model", optim_key="optim")
            else:
                logger.info(f"Loading only the model from the checkpoint.")
        ...
        while train_state.step < args.steps:
            ...
            ) or every_n_steps(train_state, args.checkpoint.eval.every, acc_step=0) or train_state.step in args.checkpoint.must_eval_steps or train_state.step == args.steps:
            ...
            ) or train_state.step in args.checkpoint.must_eval_steps or train_state.step == args.steps):
