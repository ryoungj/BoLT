...
class RotaryEmbedding(torch.nn.Module):
    ...
    def __init__(self, theta: float, head_dim: int, max_seqlen: int = 1024, rope_scaling: Optional[dict] = None):
        super().__init__()
        ...
        self.register_buffer(
            "freqs_cis",
            # precompute_freqs_cis(dim=head_dim, end=max_seqlen, theta=theta, rope_scaling=rope_scaling),
            torch.empty((self.max_seqlen, self.head_dim // 2, 2, 2)),  # init to empty tensor first
            persistent=False,
        )
    ...
    def reset_parameters(self):
        self.freqs_cis[...] = precompute_freqs_cis(
            dim=self.head_dim, end=self.max_seqlen, theta=self.theta, rope_scaling=self.rope_scaling
        )
    ...
    def forward(
        ...
        """
        assert self.is_initialized, "RotaryEmbedding is not initialized"
...
class BaseTransformer(nn.Module):
    ...
    def reset_parameters(self):
        # check if the device is meta device
        is_meta_device = next(self.parameters()).device.type == 'meta'

        if not is_meta_device:  # rope scaling cannot be done on meta device
