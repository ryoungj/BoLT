...
from lingua.tokenizer import build_tokenizer, TokenizerArgs, SpecialTokens
import numpy as np
import logging
...
VAL_DATA_FILE_PATTERN = "*.val.jsonl"
...
class TokenizerState(TypedDict):
    ...
    fall_back_to_raw: Optional[bool]
    mix_in_raw_ratio: Optional[float]
    rng_state: Dict[str, Any]
...
def loop_on_jsonl(
    ...
    try:
        while True:
            # TODO: we many need to do reshuffle here after each iteration
...
def tokenize(
    ...
    fall_back_to_raw: bool = False,
    mix_in_raw_ratio: float = 0.0,
    ...
    for content, state in iterator:
        ...
        if latent_type is None or (mix_in_raw_ratio > 0.0 and rng.random() < mix_in_raw_ratio):
            use_raw = True
        else:
            if latent_type == "pure":
                if "latent" in content: 
                    text = content["latent"]   # pure latent/synthetic data as text
                elif "chunks" in content and len(content["chunks"]) > 0:
                    chunks = content["chunks"]
                    separator = ""
                    if add_eos:
                        separator += tokenizer.decode([tokenizer.eos_id])
                    if add_bos:
                        separator += tokenizer.decode([tokenizer.bos_id])
                    text = separator.join([chunk["latent"] for chunk in chunks])
                elif fall_back_to_raw:
                    use_raw = True
                else:
                    logger.warning(f"Latent type pure but no latent key found, skipping this sample")
                    continue
            else:
                if "chunks" not in content or len(content["chunks"]) == 0:
                    if fall_back_to_raw:
                        use_raw = True
                    else:
                        raise ValueError(f"Latent type {latent_type} but no chunks key found!")
                else:
                    chunks = content["chunks"]
                    
                    if latent_type == "random":  # randomly insert the latent tokens before or after the suffix text
                        text = ""
                        for chunk in chunks:
                            chunk_latent = chunk["latent"]
                            chunk_suffix = chunk["suffix"]
                            is_prior = rng.random() < 0.5
                            if is_prior:
                                chunk_latent = SpecialTokens.START_OF_LATENT + SpecialTokens.PRIOR_PREFIX + chunk_latent + SpecialTokens.END_OF_LATENT
                                chunk_text = chunk_latent + chunk_suffix
                            else:
                                chunk_latent = SpecialTokens.START_OF_LATENT + SpecialTokens.POSTERIOR_PREFIX + chunk_latent + SpecialTokens.END_OF_LATENT
                                chunk_text = chunk_suffix + chunk_latent
                                chunk_text = SpecialTokens.START_OF_LATENT + SpecialTokens.END_OF_LATENT + chunk_text  # add empty latent tokens to denote the chunk text boundary
                            
                            text += chunk_text
                    elif latent_type == "prior_only":
                        text = "".join([SpecialTokens.START_OF_LATENT + chunk["latent"] + SpecialTokens.END_OF_LATENT + chunk["suffix"] for chunk in chunks])
                    elif latent_type == "posterior_only":
                        text = "".join([chunk["suffix"] + SpecialTokens.START_OF_LATENT + chunk["latent"] + SpecialTokens.END_OF_LATENT for chunk in chunks])
                    else:
        ...
        yield tokens, TokenizerState(
            ...
            fall_back_to_raw=fall_back_to_raw,
            mix_in_raw_ratio=mix_in_raw_ratio,
...
def find_and_sanitize_chunks(dataset_path: str, world_size: int, file_pattern: str = TRAIN_DATA_FILE_PATTERN):
    ...
    if n_chunks > world_size:
        ...
        logger.warning(f"WARNING: Discarding {n_discard} chunks from {n_chunks} chunks because world size {world_size} is smaller than the number of chunks {n_chunks}!!!")
    else:
        assert (
...
def build_dataloader(
    ...
    data_it = tokenize(
        ...
        tokenizer_state["latent_type"],
        tokenizer_state.get("fall_back_to_raw", False),
        tokenizer_state.get("mix_in_raw_ratio", 0.0),
        tokenizer_state["rng_state"],
    )
...
class DataArgs:
    ...
    latent_type: Optional[str] = None
    fall_back_to_raw: bool = False    # whether fall back to use raw text if latent does not exist (in some sources of the data)
    mix_in_raw_ratio: float = 0.0
...
def init_dataloader_state_from_args(
    ...
    return init_state(
        ...
        fall_back_to_raw=args.fall_back_to_raw,
        mix_in_raw_ratio=args.mix_in_raw_ratio,
    )
